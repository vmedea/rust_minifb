...
use super::xkb_ffi::{self, xkb_keysym_to_utf32, xkb_context_new, xkb_context_unref, xkb_keymap_new_from_string, xkb_keymap_unref, xkb_state_key_get_one_sym, xkb_state_new, xkb_state_unref, xkb_state_update_mask};
...
impl Window {
    pub fn new(name: &str, width: usize, height: usize, opts: WindowOptions) -> Result<Self> {
        ...
        let context = unsafe {
            xkb_context_new(xkb_ffi::xkb_context_flags::XKB_CONTEXT_NO_FLAGS)
        };
        if context.is_null() {
            panic!("Could not create xkb context.");
        }
        ...
        for event in self.input.iter_keyboard_events() {
            ...
            match event {
                Event::Keymap { format, fd, size } => {
                    let keymap = Self::handle_keymap(self.xkb_context, format, fd, size);
                    self.xkb_keymap = keymap;
                    self.xkb_state = unsafe {
    ...
    fn handle_key(
        ...
        let key_xkb = unsafe { xkb_state_key_get_one_sym(keymap_state, key) };
        if key_xkb != 0 {
            use super::xkb_keysyms as key;

            if state == wl_keyboard::KeyState::Pressed {
                // Taken from GLFW
                let code_point = unsafe { xkb_keysym_to_utf32(key_xkb) };
    ...
    fn handle_keymap(context: *mut xkb_ffi::xkb_context, keymap: KeymapFormat, fd: RawFd, len: u32) -> *mut xkb_ffi::xkb_keymap {
        match keymap {
            KeymapFormat::XkbV1 => {
                unsafe {
                    ...
                    if addr == libc::MAP_FAILED {
                        panic!("Could not mmap keymap from compositor");
                    }
